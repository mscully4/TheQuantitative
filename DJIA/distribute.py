...
def to_pdf(filename):
    ...
    pdfkit.from_file(filename, 'DailyDJIAReport.pdf', options=options)
    infile = PdfFileReader('DailyDJIAReport.pdf', 'rb')
    output = PdfFileWriter()
    output.addPage(infile.getPage(0))
    with open('DailyDJIAReport.pdf', 'wb') as fh:
        output.write(fh)
...
def send(filename):
    ...
    msg['From'] = fromaddr
    msg['To'] = toaddr
    msg['Subject'] = "Daily Dow Jones Industrial Report"
