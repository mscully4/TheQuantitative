...
def render(df, bands, template, index):
    ...
    bands.columns = ["Currency","Close","MA","STD","Upper","Lower"]
    technicals = bands.drop('STD', axis=1).to_html(index=False, table_id="bands", border=0, float_format=lambda x: '%.2f' % x)
